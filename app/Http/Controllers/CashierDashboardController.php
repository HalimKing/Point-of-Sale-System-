...
class CashierDashboardController extends Controller
{
    ...
    private function getCashierInfo($cashier): array
    {
        ...
        $totalShifts = Sales::where('user_id', $cashier->id)
    ...
    private function getShiftMetrics($cashier, string $timeRange): array
    {
        ...
        $todaySales = Sales::where('user_id', $cashier->id)
            ->whereDate('created_at', today())
            ->sum('grand_total');

        // Total Transactions for time range
        $totalTransactions = Sales::where('user_id', $cashier->id)
        ...
        // Refunds processed
        $refunds = Sales::where('user_id', $cashier->id)
            ->whereDate('created_at', today())
            ->count();
        ...
        $previousDaySales = Sales::where('user_id', $cashier->id)
        ...
        $previousDayTransactions = Sales::where('user_id', $cashier->id)
            ->whereDate('created_at', today()->subDay())
            ->count();
    ...
    private function getShiftSalesTrend($cashier, string $timeRange): array
    {
        ...
        $salesData = Sales::where('user_id', $cashier->id)
            ->whereBetween('created_at', $dateRange)
            ->select(
    ...
    private function getDailyPerformance($cashier): array
    {
        ...
        $dailySales = Sales::where('user_id', $cashier->id)
    ...
    private function getPaymentMethods($cashier, string $timeRange): array
    {
        ...
        $payments = Sales::where('user_id', $cashier->id)
            ->whereBetween('created_at', $dateRange)
            ->select(
                'payment_method',
